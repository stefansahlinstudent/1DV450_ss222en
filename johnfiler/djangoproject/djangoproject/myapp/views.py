...
def login_user(request):
	...
	if request.method == "POST":
		...
		if form.is_valid():
			...
			#user = authenticate(email=email_to_try, password=password_to_try)
			if user is not None:
				if user.is_active:
					login(request, user)
...
def logout_user(request):
	...
	return redirect('login') #This one should go to logout_user but gets some kind of error. 
	#form = LoginForm(request.POST)
...
def user_add(request):	
	...
	if form.is_valid():
			#user.create #How to get the variables here. 
			form.save()
			return redirect('project_list')
	else:
		form = UserForm()
...
def minigoal_add(request):
	...
	if form.is_valid():
			#form.instance.start = datetime.date.today()
			#form.instance.end = datetime.date.today()
			#How you add projectOwner for exampel
